...
internal class DeviceObservatory(private val context: Context) {
    ...
    private fun sensors(): List<ObservatoryItem> {
        val sm = context.getSystemService(Context.SENSOR_SERVICE) as android.hardware.SensorManager; val sensors = sm.getSensorList(android.hardware.Sensor.TYPE_ALL); val names = sensors.take(20).joinToString("; ") { it.name }
        return listOf(ObservatoryItem("Sensors", "Exposed sensors", sensors.size.toString()), ObservatoryItem("Sensors", "Sensor names", names.ifBlank { "None exposed" }), ObservatoryItem("Privacy", "Microphone global access toggle", "Not exposed through stable third-party API", "NOT_EXPOSED_BY_ANDROID"), ObservatoryItem("Privacy", "Camera global access toggle", "Not exposed through stable third-party API", "NOT_EXPOSED_BY_ANDROID"), ObservatoryItem("Privacy", "Active mic/camera app identity", "Not exposed to ordinary third-party apps", "RESTRICTED BY_ANDROID"))
    }
    ...
    private fun apps(): List<ObservatoryItem> {
        ...
        val grants = userApps.flatMap { pkg -> val requested = pkg.requestedPermissions ?: return@flatMap emptyList(); requested.filter { it in sensitive && pm.checkPermission(it, pkg.packageName) == PackageManager.PERMISSION_GRANTED }.map { "${pkg.packageName}:$it" } }.sorted()
