...
private class WatchStore(context: Context) {
    ...
    fun exportReport(): String = buildString {
        val history = events(); val signals = detectSignals(history); appendLine("POCKET WATCHTOWER v$VERSION"); appendLine("Owner Forensic Observatory"); appendLine("Generated: ${formatter.format(Date())}"); appendLine("Integrity: ${if (verify()) "VERIFIED" else "INTEGRITY FAILURE"}"); appendLine("Evidence chain: ${if (verify()) "VERIFIED" else "BROKEN"}"); appendLine("Snapshot SHA-256: ${snapshotHash()}"); appendLine("Events: ${history.size}"); appendLine("Correlated signals: ${signals.size}"); appendLine()
        ...
        if (signals.isNotEmpty()) { appendLine("SIGNALS & WHY IT MATTERS"); signals.forEachIndexed { index, signal -> appendLine("Signal ${index + 1}: ${signal.level}"); appendLine("  Window: ${signal.window}"); appendLine("  Why it matters: ${signal.whyItMatters}"); appendLine("  Possible reasons:"); signal.possibleReasons.forEach { appendLine("    - $it") }; appendLine("  What would strengthen it: ${signal.whatWouldStrengthen}"); appendLine("  Limitation: ${signal.limitation}"); appendLine() } }
            ...
            Scaffold(topBar = { TopAppBar(title = { Text("Pocket Watchtower V$VERSION") }) }) { padding ->
                ...
                LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    if (signals.isNotEmpty()) {
                        item { Text("SIGNALS & WHY IT MATTERS", style = MaterialTheme.typography.titleLarge); Text("A signal is a correlated group of observations. It is not an accusation or a finding of compromise.", style = MaterialTheme.typography.bodySmall) }
                        items(signals) { signal -> Card(Modifier.fillMaxWidth().clickable { selectedSignal = signal }) { Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) { Text(signal.level, style = MaterialTheme.typography.titleMedium); Text(signal.title); Text("${signal.window}", style = MaterialTheme.typography.bodySmall); Text("${signal.observations.size} related observation(s) • tap for why it matters", style = MaterialTheme.typography.bodySmall) } } }
                    } else { item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) { Text("SIGNALS & WHY IT MATTERS", style = MaterialTheme.typography.titleMedium); Text("No correlated signal detected in the current evidence history."); Text("This does not prove the device is uncompromised. It means Watchtower has not observed a qualifying correlation.", style = MaterialTheme.typography.bodySmall) } } } }
