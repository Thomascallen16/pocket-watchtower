...
private class WatchStore(context: Context) {
            ...
            for (i in 0 until a.length()) {
                ...
                add(
                    Event(
                        ...
                        if (o.isNull("previous")) null else o.optString("previous"),
                        o.optString("current"),
                        o.optString("hash")
                    )
                )
    ...
    fun snapshotHash(): String = sha256(
        snapshot().toSortedMap().entries.joinToString("|") { "${it.key}=${it.value}" }
    )
    ...
    private fun hashEvent(event: Event, previousHash: String) = sha256(
        listOf(
            previousHash,
            event.time,
            event.key,
...
class MainActivity : ComponentActivity() {
    ...
    private fun App() {
        MaterialTheme {
            Scaffold(
                topBar = { TopAppBar(title = { Text("Pocket Watchtower V$VERSION") }) }
            ) { padding ->
                ...
                LazyColumn(
                    ...
                    item {
                        Card(Modifier.fillMaxWidth()) {
                            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                                Text("DEVICE OBSERVATORY", style = MaterialTheme.typography.labelLarge)
                    ...
                    item {
                        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                            sections.take(4).forEach { section ->
                                OutlinedButton(onClick = { selectedSection = section }) {
                                    Text(if (selectedSection == section) "• $section" else section)
                                }
                            }
                        }
                    }
                    ...
                    item {
                        Card(Modifier.fillMaxWidth()) {
                            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                Text("Visibility Map", style = MaterialTheme.typography.titleMedium)
                                Text("KNOWN / OBSERVED — Android exposed it.")
                                Text("USER-GRANTED / REQUIRES SPECIAL ACCESS — access depends on the owner.")
                                Text("RESTRICTED BY ANDROID — the app cannot see everything.")
                    ...
                    item {
                        Text(
                            "Watchtower reports observable state. It does not prove who caused a change, cannot see provider-side records, and cannot guarantee that a device is uncompromised.",
